"""Higgs v3 TTS app fuer VoiceNook — wird in-process in den Hauptserver gemountet.

- Modell wird lazy geladen und nach Inaktivitaet wieder entladen (RAM freigeben).
- Kein separater Prozess/Port mehr: dieser FastAPI-App wird unter /higgs gemountet.
"""
import io
import os
import re
...
SR = 24000
HOP = 960
DEFAULT_MODEL = "whitelabel/mlx-q6-higgs-tts-3-4b"
IDLE_TIMEOUT_SECONDS = 300  # Standard: 5 Minuten Inaktivitaet -> Modell entladen

app = FastAPI(title="VoiceNook Higgs")
...
def configure(model=None, voice_dir=None, idle_minutes=None):
    """Wird vom Hauptserver beim Start aufgerufen, um das Higgs-Modul zu konfigurieren."""
    global _args, VOICE_DIR, IDLE_TIMEOUT_SECONDS
    if _args is None:
        import argparse
        _args = argparse.Namespace(model=model or DEFAULT_MODEL)
    if model:
        _args.model = model
    if voice_dir:
        VOICE_DIR = os.path.abspath(voice_dir)
    if idle_minutes is not None:
        IDLE_TIMEOUT_SECONDS = max(1, int(idle_minutes)) * 60
...
def get_model():
    """Lae dt das Higgs-Modell lazy (thread-sicher)."""
    global _model
    with _LOAD_LOCK:
        if _model is None:
...
def unload_model():
    """Entlaedt das Higgs-Modell und gibt den RAM frei (Prozess bleibt)."""
    global _model, _sessions
    with _LOAD_LOCK:
        if _model is not None:
            ...
            import gc
            gc.collect()
            print("[*] Higgs-Modell entladen (RAM freigegeben).", flush=True)
...
def main():
    """Standalone-Ausfuehrung (optional, z.B. zum Debuggen). Normalerweise wird die
    App in den Hauptserver gemountet und dieser Server nicht separat gestartet."""
    import argparse
    import uvicorn
    global _args, VOICE_DIR, IDLE_TIMEOUT_SECONDS
    p = argparse.ArgumentParser(description="VoiceNook Higgs (standalone)")
